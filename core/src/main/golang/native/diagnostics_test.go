...
func TestDiagnosticsProbeAuthenticatesController(t *testing.T) {
	...
	if !probeDiagnosticsController(server.Client(), server.URL, secret) {
		t.Fatal("authenticated controller was not ready")
	}
}
...
func TestDiagnosticsProbeRejectsUnauthorizedController(t *testing.T) {
	...
	if probeDiagnosticsController(server.Client(), server.URL, "wrong-secret") {
		t.Fatal("unauthorized controller was reported ready")
	}
}
...
func TestDiagnosticsStatusReportsFailureWithoutInternalDetails(t *testing.T) {
	...
	if !strings.Contains(status, `"failed":true`) {
		t.Fatalf("status did not report failure: %s", status)
	}
	if strings.Contains(status, "error") || strings.Contains(status, "недоступна") || strings.Contains(status, "example.com") {
		t.Fatalf("status exposed internal error: %s", status)
	}

	diagnosticsStop()
	if status = diagnosticsQuery(); !strings.Contains(status, `"failed":false`) {
